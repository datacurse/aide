...
const EMPTY: AppLocation = { projectId: null, pane: "tasks", taskId: null, sessionId: null }

export function parseLocation(hash: string): AppLocation {
  // "#/p/<projectId>/tasks/<taskId>" or "#/p/<projectId>/chats/<sessionId>"
  const parts = hash.replace(/^#\/?/, "").split("/").filter(Boolean)
  if (parts[0] !== "p" || !parts[1]) return EMPTY

  const pane: Pane = parts[2] === "chats" ? "chats" : "tasks"
...
export function formatLocation(loc: AppLocation): string {
  ...
  const id = loc.pane === "chats" ? loc.sessionId : loc.taskId
...
export function useAppLocation(): [AppLocation, (patch: Partial<AppLocation>) => void] {
  ...
  const navigate = useCallback((patch: Partial<AppLocation>) => {
    setLoc((prev) => {
      ...
      // or session id from another project resolves to nothing.
