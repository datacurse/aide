 ...
 *
 * It is line 3 or 4 — `user.message`, then a pre-turn sweep's `commit.landed`
 * when the tree was dirty, `checkpoint.taken`, then the SDK's init. The bound
 * is not an optimisation, it is a refusal to read a whole log looking for an
 * event that a turn which died before it spawned is never going to have.
 */
