 ...
 * about how the run went. The reading is the human's job, and the profile exists
 ...
 * of inside it.
 *
 * A profile, not a receipt, which is what this was called first. That name put
 * the one number you can do least about — an estimated dollar figure, off a price
 * table bundled into the SDK — in the title, and left out the one you can act on:
 * where the wall clock went. `profile` is the word a programmer already has for
 * that, and renaming it is only half the fix, which is why "Round trips" below
 * exists at all.
 */
import type { ModelSpend, Profile, Project, RunEvent, RunStatus } from "@aide/protocol"
 ...
 * end stamp; summing them reports three times the elapsed time and a profile
...
function describeInput(input: unknown): string {
  ...
  // backticks with nothing between them, which reads as a bug in the profile
 ...
 * run-history queries hurt — but a profile that reuses the list's cache pays for
...
function dur(ms: number): string {
  ...
  // second of a minute, which is how a profile ends up saying a number that does
  // not exist.
...
function render(opts: { project: Project; sessionId: string; runs: RunSummary[] }): string {
  ...
  // Zero for a conversation made only of non-agent runs — `chat.ts` writes
  // `numTurns: 0` for a commit, which has no SDK steps to count — so everything
  // derived from it below is behind a guard rather than dividing by it.
  const steps = runs.reduce((n, r) => n + r.steps, 0)
  ...
  out.push(`# aide profile — ${titleOf(runs)}`)
  out.push("")
  ...
  runs.forEach((run, i) => {
    ...
    // is what a turn in flight looks like — and asking for a profile mid-turn is
  ...
  // The section the rename was for. Model time is the bulk of the wall clock on
  // almost every turn, and it is round trips × context — but the document used to
  // report the total and stop, so the reading it invited was "the model thought
  // for sixteen minutes" when the answer was "you took forty steps". Calls per
  // step is the lever: independent calls batched into one message cost one round
  // trip, sent one per message they cost one each at the same context.
  if (steps > 0) {
    out.push("## Round trips")
    out.push("")
    out.push("| | |")
    out.push("| --- | --- |")
    out.push(`| model steps | ${steps} |`)
    out.push(`| tool calls per step | ${(calls.length / steps).toFixed(1)} |`)
    out.push(`| model time per step | ${dur(modelMs / steps)} |`)
    out.push(
      `| model time | ${dur(modelMs)} of ${dur(wallMs)} wall clock${wallMs ? ` · ${Math.round((modelMs / wallMs) * 100)}%` : ""} |`,
    )
    out.push("")
    out.push("_A step is one round trip: the whole context up, one reply down. It is the unit")
    out.push("the wall clock is actually made of, and batching independent calls into a single")
    out.push("message is the only thing that reduces how many you take._")
    out.push("")
  }
...
/** The profile for one conversation. Always answers; no runs is an answer. */
export async function conversationProfile(
  log: EventLog,
  project: Project,
  sessionId: string,
): Promise<Profile> {
